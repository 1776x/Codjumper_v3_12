...
spawnPlayer()
{
	...
	//self thread maps\mp\gametypes\_hardpoints::hardpointItemWaiter(); // [CJ] [//]
...
gameTimer()
{
	...
	//level waittill("prematch_over"); // Drofder
...
Callback_StartGameType()
{
	...
	//thread maps\mp\gametypes\_damagefeedback::init(); // [CJ] [//]
	...
	//thread maps\mp\gametypes\_hardpoints::init(); // [CJ] [//]
...
finishPlayerDamageWrapper( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	else if(isDefined(eAttacker) && eAttacker.cj["status"] >= 1 && eAttacker != self && self.cj["status"] < 1 && sMeansOfDeath != "MOD_FALLING" && sMeansOfDeath != "MOD_PROJECTILE_SPLASH" && sMeansOfDeath != "MOD_PROJECTILE" && sMeansOfDeath != "MOD_MELEE")
		self finishPlayerDamage( eInflictor, eAttacker, 999, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime );
		...
		else
		{
			...
			else
			{
				...
				/*if ( isDefined( level.hardpointItems ) && isAlive( attacker ) )
					attacker thread maps\mp\gametypes\_hardpoints::giveHardpointItemForStreak();*/ // [CJ] [//]
...
Callback_PlayerLastStand( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration )
{
	...
	for( i = 0; i < weaponslist.size; i++ )
	{
		...
		if ( maps\mp\gametypes\_weapons::isPistol( weapon ) )
		{
			...
			self takeallweapons();
			// self maps\mp\gametypes\_hardpoints::giveOwnedHardpointItem(); // [CJ] [//]
